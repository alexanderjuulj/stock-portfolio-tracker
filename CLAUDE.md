...
- Design tokens are CSS custom properties on `:root`: colors in `base/_colors.scss` (`--clr-accent` growth green + `--clr-accent-strong`, `--clr-gold` secondary, green-tinted slate grays, `--background-color-tertiary` for nested rows, `--clr-backdrop`; losses use `--text-clr-error` red from `base/_typography.scss`), typography in `base/_typography.scss` (`--font-*`, `--fs-*`, `--fw-*`, `--lh-*`).
