...
Features so far: a **dashboard** (`/`) with one row per stock — amount summed across accounts, average buy price, live market price, profit (abs + %), market value (native + EUR), portfolio weight, day change % (vs. previous close), sector, personal notes — that expands into the per-account purchase lots (with dates), plus totals (open and realized profit, free cash with its share of the portfolio), account filter tabs, a **sector donut chart** (`SectorChart`, plain SVG, hover to isolate a slice), **selling** (pick the account, amount, price prefilled from the quote, date, reason; proceeds can be credited to the account's free cash), and an **Accounts** section showing free cash per account (native + EUR), stocks value and total, with an all-accounts total; a **history page** (`/history`) listing every purchase and sale with its realized profit/loss and a per-stock×account summary; a **settings page** (`/settings`) for the quote provider / API key and DB backup import/export, and a **concept mode** (flask toggle in the header): a what-if sandbox where everything — prices, lots, accounts, cash — can be changed freely without touching the real portfolio, and is discarded on leave. See "Data model", "Ledger", "Market data" and "Concept mode" under Data layer.
...
- **Concept mode**: the sandbox is a *full copy of the DB* (`data/rahamasin.concept.db`, forked with `VACUUM INTO` on enter via `POST /api/concept {active}`, deleted on leave). While on, the client sends `x-concept: 1` on every request (`src/lib/api.ts`) and `server/db.ts` routes `getDb()` to the copy through an `AsyncLocalStorage` request context — so every feature works in the sandbox with zero per-route code. Export/import always target the real DB (import is refused while the header is set). Hand-set prices live in `quote_overrides` (migration 005): `PUT|DELETE /api/quotes/:ticker` — rejected outside concept context, so the real DB's table stays empty — and an overridden ticker is pinned in `getQuotes`/`peekQuotes` (never fetched, never stale, `priceOverridden` on the position; day change still uses the cached previous close). Client state is `src/lib/concept.ts` (localStorage `rahamasin.concept` + `data-concept` on `<html>`, applied pre-paint in index.html); flipping it remounts the page tree via the `Routes` key in `App.tsx` so all data refetches. UI: `ConceptToggle` in the header (confirm on leave — leaving discards), amber "Concept" badge by the brand, gray "concept desk" ground (`night-concept`/`day-concept` mixins in `base/_colors.scss` — only paper/surface tokens shift), "Set price"/"Reset price" in the holdings row menu (`PriceForm`), and an amber "set" marker on hand-set prices.
